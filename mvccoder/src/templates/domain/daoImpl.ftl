...
public class ${domain.name}DaoImpl extends BaseDaoImpl<${domain.name},<#if domain.isMany2ManyKey>${domain.name}<#elseif !domain.isMany2ManyKey && (domain.id.type==statics["com.wanmei.domain.FieldHelper"].TYPE_MANY2ONE)>${domain.id.entityName}<#else>${domain.id.primaryType}</#if>> implements ${domain.name}Dao{
