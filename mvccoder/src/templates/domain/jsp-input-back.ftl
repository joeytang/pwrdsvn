...
$(document).ready(function(){
	...
	$("#${f.name}Id").formValidator({
		<#if !f.nullable>empty :false,</#if>autoModify:true,onShow:"请输入0-10000之间整数",onFocus:"请输入0-10000之间整数",onCorrect:"恭喜您,输入正确"
	}).inputValidator({
		max:10000,type:"value",onError:"请输入0-10000之间整数"
	});
	<#elseif (f.type==statics["com.wanmei.domain.FieldHelper"].TYPE_FLOAT) || (f.type==statics["com.wanmei.domain.FieldHelper"].TYPE_DOUBLE)>
	$("#${f.name}Id").formValidator({
		<#if !f.nullable>empty :false,</#if>autoModify:true,onShow:"请输入0-10000之间数字",onFocus:"请输入0-10000之间数字",onCorrect:"恭喜您,输入正确"
	}).inputValidator({
		max:10000,type:"value",onError:"请输入0-10000之间数字"
	});	
