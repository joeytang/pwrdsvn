 ...
 * ${domain.label}dao接口
 */
public interface ${domain.name}Dao extends BaseDao<${domain.name},<#if domain.isMany2ManyKey>${domain.name}<#elseif !domain.isMany2ManyKey && (domain.id.type==statics["com.wanmei.domain.FieldHelper"].TYPE_MANY2ONE)>${domain.id.entityName}<#else>${domain.id.primaryType}</#if>> {
}
