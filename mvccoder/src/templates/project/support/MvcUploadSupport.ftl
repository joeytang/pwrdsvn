...
public class MvcUploadSupport {
	...
	public static final long DEFAULT_MAX_SIZE = (long)(100 * 1024 * 1024);//默认上传文件最大限制100M
	/**
	...
	public JSONObject upload(MultipartFile file, HttpServletRequest request,
		...
		JSONObject json = null;
		String path = destDir;
		String wRt = getWebRoot(request);
		if(!wRt.endsWith("/") || !wRt.endsWith("\\")){
			wRt+="/";
		}
		destDir = wRt + destDir;
		...
		try {
			...
			String ext = FileUtil.getExtention(oriName);
			if (StringUtils.isBlank(destFileName)) {
				destFileName = new Date().getTime() + "_" + oriName;
			}
